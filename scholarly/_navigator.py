...
from ._proxy_generator import (DOSException, MaxTriesExceededException,
                               ProxyGenerator)
...
class Navigator(object, metaclass=Singleton):
    ...
    def set_logger(self, enable: bool):
        ...
        print((logging.INFO if enable else logging.CRITICAL))
        ...
        while tries < self._max_retries:
            try:
                ...
                if (
                    ...
                    print("Session proxy config is {}".format(pm._proxies))
                ...
                if resp.status_code == 200 and not has_captcha:
                    ...
                    print("Got a 404 error. Attempting with same proxy")
                    tries += 1
                    continue
                elif has_captcha:
                    ...
                    print("Got an access denied error (403).")
                    if not pm.has_proxy():
                        print("No other connections possible.")
                        if not self.got_403:
                            print("Retrying immediately with another session.")
                        else:
                        ...
                        print("We can use another connection... let's try that.")
                ...
                if not pm.has_proxy():
                    print("No other connections possible.")
                    w = random.uniform(60, 2 * 60)
                    print("Will retry after %.2f seconds (with the same session).", w)
                    time.sleep(w)
                    continue
                ...
                if timeout < 3 * self._TIMEOUT:
                    print("Increasing timeout and retrying within same session.")
                    timeout = timeout + self._TIMEOUT
                    continue
                print("Giving up this session.")
                ...
                print(err)
                print("Retrying with a new session.")
    ...
    def search_authors(self, url: str) -> Author:
        ...
        while True:
            ...
            print("Found %d authors", len(rows))
            for row in rows:
            ...
            if next_button and "disabled" not in next_button.attrs:
                print("Loading next page of authors")
                url = next_button["onclick"][17:-1]
                url = codecs.getdecoder("unicode_escape")(url)[0]
                soup = self._get_soup(url)
            else:
                print("No more author pages")
                break
